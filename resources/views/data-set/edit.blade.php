                    ...
                    <form action="{{ route('data-set.update', $dataSet->id) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="nama_bangunan" type="text" name="nama_bangunan" :value="old('nama_bangunan', $dataSet->nama_bangunan)" required autofocus />
                            </div>
                            <div>
                                ...
                                <select name="jenis_konstruksi" id="jenis_konstruksi" 
                                    ...
                                    @foreach(['Gedek', 'Semi Permanen', 'Permanen', 'Permanen Bertingkat'] as $jenis)
                                        <option value="{{ $jenis }}" {{ old('jenis_konstruksi', $dataSet->jenis_konstruksi) == $jenis ? 'selected' : '' }}>
                                            {{ $jenis }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                                <h3 class="text-sm font-bold text-gray-800 tracking-wide uppercase">Nilai Kondisi Komponen</h3>
                                <span class="text-xs text-slate-500 font-medium">(Skala: 5 = Sangat Baik, 1 = Rusak Berat)</span>
                            </div>
                                    ...
                                    <div>
                                        ...
                                        <select name="{{ $field }}" id="{{ $field }}" 
                                            ...
                                            @for ($i = 5; $i >= 1; $i--)
                                                ...
                                                @endphp
                                                <option value="{{ $i }}" {{ old($field, $dataSet->$field) == $i ? 'selected' : '' }}>
                                                    {{ $labelKondisi }}
                                                </option>
                                            @endfor
                                        </select>
                        ...
                        <div>
                            ...
                            <select name="kategori" id="kategori" 
                                ...
                                @foreach(['Layak', 'Kurang Layak', 'Tidak Layak'] as $kat)
                                    <option value="{{ $kat }}" {{ old('kategori', $dataSet->kategori) == $kat ? 'selected' : '' }}>
                                        {{ $kat }}
                                    </option>
                                @endforeach
                            </select>
