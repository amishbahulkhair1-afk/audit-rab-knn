...
<div class="max-w-6xl mx-auto py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('buildings.index') }}" class="bg-slate-200 text-slate-700 px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-slate-300 transition">
            Kembali
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Kode & Nama</label>
            ...
            <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Rayon & Luas</label>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4 text-center">
                        <a href="{{ route('audits.show', $audit->id) }}" class="text-emerald-600 font-semibold hover:underline">Lihat</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="4" class="p-8 text-center text-slate-400">Belum ada riwayat audit.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div id="modalEdit" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
            ...
            <button onclick="document.getElementById('modalEdit').classList.add('hidden')" class="text-slate-400 hover:text-slate-600">✕</button>
        </div>
        <form action="{{ route('buildings.update', $building->id) }}" method="POST">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="nama_bangunan" value="{{ $building->nama_bangunan }}" class="w-full border border-slate-200 rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500 outline-none">
                </div>
                <!-- Tambahkan field lainnya dengan class yang sama -->
            </div>
            <div class="px-6 py-4 bg-slate-50 flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('modalEdit').classList.add('hidden')" class="px-4 py-2 text-sm font-semibold text-slate-600">Batal</button>
                <button type="submit" class="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
