...
<div class="py-10">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-slate-500 mt-1">{{ __('Lakukan penilaian kondisi komponen bangunan untuk proses klasifikasi.') }}</p>
        </div>
        ...
        <form action="{{ route('audits.store') }}" method="POST" class="space-y-8">
            ...
            <div class="bg-white p-8 shadow-xs sm:rounded-xl border border-slate-100">
                ...
                <select name="building_id" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" 
            ...
            <div class="bg-white p-8 shadow-xs sm:rounded-xl border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @foreach ($komponen as $name => $label)
                        <div class="space-y-1.5">
                            ...
                            <select name="{{ $name }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                ...
                <div class="mt-8">
                    ...
                    <textarea name="catatan" rows="3" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" placeholder="Tambahkan catatan tambahan..."></textarea>
                </div>
            </div>

            <!-- Action -->
            <div class="flex justify-end pt-4">
                <button type="submit" class="px-8 py-3 bg-emerald-600 text-white text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-emerald-700 shadow-sm transition">
                    {{ __('Proses Audit KNN') }}
                </button>
            </div>
        </form>
