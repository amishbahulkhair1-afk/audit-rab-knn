...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 pb-8 border-b border-slate-100">
        ...
        <div>
            ...
            <p class="text-lg font-bold text-slate-800">{{ \Carbon\Carbon::parse($rab->tanggal_rab)->format('d M Y') }}</p>
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-4 py-3 text-right text-slate-600">{{ number_format($detail->volume, 2, ',', '.') }}</td>
                    <td class="px-4 py-3 text-right text-slate-600">Rp {{ number_format($detail->harga_satuan, 0, ',', '.') }}</td>
            ...
            <tfoot>
                <tr class="border-t-2 border-slate-100">
                    ...
                    <td class="px-4 py-4 text-right font-bold text-emerald-600 text-lg">Rp {{ number_format($rab->total_biaya, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
