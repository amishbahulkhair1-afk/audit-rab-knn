<x-app-layout>
    ...
    <div class="space-y-5">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <h1 class="text-xl md:text-2xl font-bold leading-tight">
                            Edit: {{ $support_cost->nama_biaya }}
                        </h1>
                        <p class="text-xs md:text-sm text-white/80 mt-1">
                            Perbarui rincian pos biaya dan tarif operasional pendukung.
                        </p>
                    </div>
        ...
        <div class="glass-surface rounded-3xl overflow-hidden p-6 md:p-8">
            ...
            <form action="{{ route('support-costs.update', $support_cost) }}" method="POST" class="space-y-6 max-w-3xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nama_biaya" value="{{ old('nama_biaya', $support_cost->nama_biaya) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="kategori"
                            class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-cyan-400 focus:ring-2 focus:ring-cyan-200/50 outline-none transition"
                            required>
                            <option value="Transportasi" {{ old('kategori', $support_cost->kategori) == 'Transportasi' ? 'selected' : '' }}>Transportasi</option>
                            <option value="Operasional" {{ old('kategori', $support_cost->kategori) == 'Operasional' ? 'selected' : '' }}>Operasional</option>
                            <option value="Lain-lain" {{ old('kategori', $support_cost->kategori) == 'Lain-lain' ? 'selected' : '' }}>Lain-lain</option>
                        </select>
                        @error('kategori')
                            <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga_satuan"
                                value="{{ old('harga_satuan', $support_cost->harga_satuan) }}" min="0" placeholder="0"
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3" placeholder="Catatan tambahan pos biaya pendukung (opsional)..."
                        class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-cyan-400 focus:ring-2 focus:ring-cyan-200/50 outline-none transition">{{ old('keterangan', $support_cost->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                </div>
