<x-app-layout>
    ...
    <div class="space-y-5">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <h1 class="text-xl md:text-2xl font-bold leading-tight">
                            Edit: {{ $building->nama_bangunan }}
                        </h1>
                        <p class="text-xs md:text-sm text-white/80 mt-1">
                            Perbarui data fasilitas bangunan, jenis konstruksi, dan lokasi.
                        </p>
                    </div>
        ...
        <div class="glass-surface rounded-3xl overflow-hidden p-6 md:p-8">
            ...
            <form action="{{ route('buildings.update', $building) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="jenis_bangunan"
                            class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200/50 outline-none transition"
                            required>
                            @foreach (['Asrama', 'Madrasah', 'Masjid', 'Gedung Serbaguna', 'Kantor'] as $jenis)
                                <option value="{{ $jenis }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="rayon" value="{{ old('rayon', $building->rayon) }}"
                            class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200/50 outline-none transition"
                            required>
                        @error('rayon')
                            <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun_berdiri"
                            value="{{ old('tahun_berdiri', $building->tahun_berdiri) }}" min="1900"
                            max="{{ date('Y') + 1 }}"
                            class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200/50 outline-none transition">
                        @error('tahun_berdiri')
                            <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" rows="3"
                            class="w-full rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200/50 outline-none transition">{{ old('alamat', $building->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
